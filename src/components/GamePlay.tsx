        ...
        <div className="choice-buttons">
          {choices.map((choice) => (
            <button
              ...
              style={{
                ...checkWbutton,
                backgroundColor: selected === choice ? colors.dark : colors.white,
      ...
      {showResult && (
        <div style={{ marginTop: 20 }}>
          <div style={{ color: isCorrect ? blue : danger, fontWeight: 'bold' }}>
            {isCorrect ? '🎉 정답입니다! 포켓몬박사신가요? 🎉' : `❌ 앗! 정답은 ${pokemon?.name} 였답니다~ ❌`}
          </div>
          <br />
          <button style={{...likeBbutton, fontWeight: 'bold'}} onClick={handleNext}>다음 문제</button>
          <button style={{...grayBbutton, fontWeight: 'bold'}} onClick={() => onFinish(score)}>그만하기</button>
        </div>
      )}
    </div>
  )
